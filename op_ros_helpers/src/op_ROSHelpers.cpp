...
//		}
		...
		visualization_msgs::Marker path_mkr = CreateGenMarker(0,0,0,0,r,g,b,0.4,iCount,"Predicted_Trajectories", visualization_msgs::Marker::LINE_STRIP);
...
void ROSHelpers::ConvertFromAutowareLaneToLocalLane(const autoware_msgs::Lane& trajectory, std::vector<PlannerHNS::WayPoint>& path)
{
	...
	for(const auto & waypoint : trajectory.waypoints)
	{
		...
		wp.pos.x = waypoint.pose.pose.position.x;
		wp.pos.y = waypoint.pose.pose.position.y;
		wp.pos.z = waypoint.pose.pose.position.z;
		try
		{
			wp.pos.a = tf::getYaw(waypoint.pose.pose.orientation);
		}
		catch (exception& ex)
		{
			ROS_ERROR("%s", ex.what());
		...
		wp.v = waypoint.twist.twist.linear.x;

		wp.gid = waypoint.gid;
		wp.laneId = waypoint.lane_id;
		wp.stopLineID = waypoint.stop_line_id;
		wp.LeftPointId = waypoint.left_lane_id;
		wp.RightPointId = waypoint.right_lane_id;
		wp.timeCost = waypoint.time_cost;
		wp.laneChangeCost = waypoint.change_flag;

		if(waypoint.wpstate.event_state == 0)
		{
			wp.custom_type = CUSTOM_AVOIDANCE_DISABLED;
		}
		else if(waypoint.wpstate.event_state == 1)
		{
		...
		else if(waypoint.direction == 2)
			wp.bDir = PlannerHNS::FORWARD_RIGHT_DIR;
		else if(waypoint.direction == 3)
			wp.bDir = PlannerHNS::BACKWARD_DIR;
		else if(waypoint.direction == 4)
			wp.bDir = PlannerHNS::BACKWARD_LEFT_DIR;
		else if(waypoint.direction == 5)
			wp.bDir = PlannerHNS::BACKWARD_RIGHT_DIR;
		else if(waypoint.direction == 6)
			wp.bDir = PlannerHNS::STANDSTILL_DIR;

		wp.cost = waypoint.cost;
        wp.actionCost.emplace_back((PlannerHNS::ACTION_TYPE)waypoint.direction, wp.cost);
